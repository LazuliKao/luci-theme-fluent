                ...
                compiler.hooks.emit.tapAsync("AddLuCIWrapperPlugin", (compilation, callback) => {
                  for (const name of Object.keys(compilation.assets)) {
                    if (name.endsWith(".js")) {
                      ...
                      const src = String(typeof asset.source === "function" ? asset.source() : asset.source);
                      ...
                      compilation.assets[name] = {
                        ...
                        map: () => null,
                        sourceAndMap: () => ({ source: sourceEdit, map: null as unknown as never }),
                        updateHash: (hash: { update(s: string): void }) => hash.update(sourceEdit),
                      } as unknown as (typeof compilation.assets)[string];
                    }
                  }
                  callback();
                });
